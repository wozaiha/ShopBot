...
const { ButtonStyle, ModalBuilder, TextInputStyle, TextInputBuilder, StringSelectMenuBuilder, EmbedBuilder } = require('discord.js');
...
const save = (db) => new Promise(async (resolve, reject) => {
    let path = (db === shops) ? shopsPath : (db === currencies) ? currenciesPath : accountsPath
    try {
	...
	async execute(interaction) {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'change-shop-name-modal') {
                const shopName = interaction.fields.getTextInputValue('shop-name-input').replace(/ /g, ' ')
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            if (interaction.customId === 'select-shop-reorder') {
                ...
                await interaction.update({ content: `Change position of **[${shopName}]** to __**${newPos}**__`, components: [new ActionRowBuilder().addComponents(selectShopMenu), buttons], ephemeral: true })
            }
        }

        if (interaction.isButton()) {
            if (interaction.customId === 'submit-shop-new-product') {
                const shopName = interaction.message.content.match(/\*\*\[[^ ]+\]\*\*/g)[0].replace(/\*|\[|\]/g, '').replace(/ /g, ' ')
                const productName = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[0].replace(/\*/g, '').replace(/ /g, ' ')
            ...
            if (interaction.customId === 'submit-currency-new-shop') {
                const shopName = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[0].replace(/\*/g, '').replace(/ /g, ' ')
                const currencyName = interaction.message.content.match(/\*\*\[[^ ]+\]\*\*/g)[0].replace(/\*|\[|\]/g, '').replace(/ /g, ' ')
            ...
            if (interaction.customId === 'submit-currency-give') {
                ...
                try {
                    ...
                    console.log(error);
                    await interaction.reply({ content: `❌ An error occured while executing this command, please try again later`, ephemeral: true })
                }
            }
            ...
            if (interaction.customId === 'submit-shop-update-product') {
                ...
                const updateOptionValue = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[2].replace(/\*/g, '').replace(/ /g, ' ')
                const selectedShop = shops.find(shop => shop.name === shopName)
            ...
            if (interaction.customId === 'submit-product-update-product') {
                ...
                const shopName = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[1].replace(/\*/g, '')
                const updateOption = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[2].replace(/\*/g, '')
                const updateOptionValue = interaction.message.content.match(/\*\*[^ ]+\*\*/g)[3].replace(/\*/g, '').replace(/ /g, ' ')
            ...
            if (interaction.customId === 'buy-button') {
                ...
                if (!selectedShop.products.length) return await interaction.reply({ content: `❌ There is no product in this shop`, components: [], ephemeral: true })
                ...
                interaction.reply({ content: `Buy **[Select Product]** from **${shopName}**`, components: [new ActionRowBuilder().addComponents(selectProductMenu), buttons], ephemeral: true })
            }
